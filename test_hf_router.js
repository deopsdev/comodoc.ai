// HF Router integration test — skips if HF_TOKEN is not set
(async () => {
    const HF = process.env.HF_TOKEN;
    if (!HF) {
        console.log('SKIP: HF_TOKEN not set. Set HF_TOKEN to run this test.');
        process.exit(0);
    }

    const fetch = global.fetch || (await import('node-fetch')).default;

    console.log('Running /chat integration test (Hugging Face Router)');
    try {
